...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ route('loan-requests.inter-department.show', $interDepartmentLoan) }}" class="btn btn-outline-secondary">Back to Loan</a>
    </div>

    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('loan-requests.inter-department.return', $interDepartmentLoan) }}" enctype="multipart/form-data" id="multiItemReturnForm">
                @csrf
                @method('PATCH')
                    ...
                    <table class="table table-bordered align-middle">
                        ...
                        <tbody>
                            @forelse($activeItems as $idx => $bi)
                                <tr>
                                    <td>
                                        <div class="fw-semibold">{{ $bi->issuedItem->supply->name }}</div>
                                        <div class="text-muted small">Issued by: {{ $bi->lendingDepartment->department_name ?? 'N/A' }}</div>
                                        <input type="hidden" name="items[{{ $idx }}][borrowed_item_id]" value="{{ $bi->id }}" />
                                    </td>
                                    <td class="text-center">{{ $bi->quantity_borrowed }}</td>
                                    <td>
                                        <input type="number" class="form-control" name="items[{{ $idx }}][quantity_returned]" min="1" max="{{ $bi->quantity_borrowed }}" value="{{ old('items.'.$idx.'.quantity_returned', $bi->quantity_borrowed) }}" />
                                    </td>
                                    ...
                                    <td class="missing-col">
                                        <input type="number" class="form-control missing-count" name="items[{{ $idx }}][missing_count]" min="0" max="{{ $bi->quantity_borrowed }}" value="{{ old('items.'.$idx.'.missing_count', 0) }}" />
                                        <div class="form-text">Missing items not returned</div>
                                    </td>
                                    <td class="damaged-col">
                                        <input type="number" class="form-control damaged-count" name="items[{{ $idx }}][damaged_count]" min="0" max="{{ $bi->quantity_borrowed }}" value="{{ old('items.'.$idx.'.damaged_count', 0) }}" />
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center text-muted">No active items to return.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    document.querySelectorAll('#multiItemReturnForm tbody tr').forEach(function(row) {
        ...
        if (missingInput) {
            function adjustReturnedFromMissing() {
                ...
                // Respect minimum of 1 returned (current form constraint)
